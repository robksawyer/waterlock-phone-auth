...
var wl = require('../../waterlock-phone-auth');
...
function handlePost(req, res, sails, params){
  ...
  if(typeof params.email !== 'undefined'){
    func = sendEmail;
  }else if(typeof params.phone !== 'undefined'){
    func = sendPhoneVerifyCode;
 ...
 * and sending a new phone verification code to the user
 * @param  object req the express request object
 * @param  object res the express response object
 * @param  waterlineDAO User  the waterline User dao
 * @param  waterlineDAO ResetToken the waterline ResetToken dao
 * @param  object params all params for this request
 */
function sendPhoneVerifyCode(req, res, sails, params){
  sails.models.auth.findOne({phone: params.phone}).populate('user').exec(function(err, a){
    if(a){
